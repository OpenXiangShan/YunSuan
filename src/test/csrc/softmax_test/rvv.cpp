...
void vfsub_vf(int vreg_dst_index, float value, int vreg_src_index,int vl){
    for(int i=0;i<vl;i++){
        cpu.vreg[vreg_dst_index][i].f=cpu.vreg[vreg_src_index][i].f-value;
    }
}
...
void vfcvt_x_f_v_i32m1(int vreg_dst_index, int vreg_src_index,int vl){
    for(int i=0;i<vl;i++){
        cpu.vreg[vreg_dst_index][i].i=(int32_t)round(cpu.vreg[vreg_src_index][i].f);
    }
}
void vfcvt_f_x_v_f32m1(int vreg_dst_index, int vreg_src_index,int vl){
    for(int i=0;i<vl;i++){
        cpu.vreg[vreg_dst_index][i].f=(float)cpu.vreg[vreg_src_index][i].i;
    }
}
...
void vfmadd_vv(int vreg_dst_index, int vreg_src1_index, int vreg_src2_index,int vl){
    for(int i=0;i<vl;i++){
        cpu.vreg[vreg_dst_index][i].f=cpu.vreg[vreg_dst_index][i].f*cpu.vreg[vreg_src1_index][i].f+cpu.vreg[vreg_src2_index][i].f;
    }
}
void vmv1r_v(int vreg_dst_index, int vreg_src_index,int vl){
    for(int i=0;i<vl;i++){
        cpu.vreg[vreg_dst_index][i].f=cpu.vreg[vreg_src_index][i].f;
    }
}

void vadd_vx(int vreg_dst_index, int vreg_src1_index, int value,int vl){
    for(int i=0;i<vl;i++){
        cpu.vreg[vreg_dst_index][i].i=cpu.vreg[vreg_src1_index][i].i+value;
    }
}
void vsll_vx(int vreg_dst_index, int vreg_src1_index, int value,int vl){
    for(int i=0;i<vl;i++){
        cpu.vreg[vreg_dst_index][i].u=(cpu.vreg[vreg_src1_index][i].u)<<value;
    }
}
void vfmul_vv(int vreg_dst_index, int vreg_src1_index, int vreg_src2_index,int vl){
    for(int i=0;i<vl;i++){
        cpu.vreg[vreg_dst_index][i].f=cpu.vreg[vreg_src1_index][i].f*cpu.vreg[vreg_src2_index][i].f;
    }
}
