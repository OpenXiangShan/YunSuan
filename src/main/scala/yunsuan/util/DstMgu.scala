...
class DstMguIO(vlen: Int) extends Bundle {
  val in = Input(new DstMguInputBundle(vlen))
  val out = Output(new DstMguOutputBundle(vlen))
}

class DstMguInputBundle(vlen: Int) extends Bundle {
  val valid = Bool()
...
class DstMguOutputBundle(vlen: Int) extends Bundle {
  val vd = UInt(vlen.W) 
}

class DstMguS1InputBundle(vlen: Int) extends Bundle {
