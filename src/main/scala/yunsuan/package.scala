...
package object yunsuan {
  ...
  object VipuType0 {
    ...
    // TODO: other op and method
    def needReverse(vipuType: UInt) =
      vipuType === sub // TODO : Modify the internal logic of VectorIntAdder to deal with this dirty code
  ...
  object VialuFixType {
    ...
    def needNoMask(fuOpType:    UInt) =
      Seq(vadc_vvm, vmadc_vvm, vmadc_vv, vsbc_vvm, vmsbc_vvm, vmsbc_vv, vmerge_vvm).map(_ === fuOpType).reduce(_ || _)
    ...
    def getSrcVdType(fuOpType: UInt, sew: UInt) = {
      ...
      val format = Mux(
        (getOpcodeGeneral(fuOpType) === getOpcodeGeneral(vadd_vv) || getOpcodeGeneral(fuOpType) === getOpcodeGeneral(
          vsub_vv
        )),
        Mux1H(
          Seq( // format for vadd/vsub : 00 vvv   01 vvw   10 wvw   11 rvvv
        ...
        Mux(
          ...
          Mux1H(
            Seq( // format for vext : 00 22v   01 44v   10 88v
          ...
          Mux1H(
            Seq( // format for general opcode : 00 vvv/0xv   01 vvm   10 mmm   11 wvv
              (formatOH === "b00".U) -> Cat(Sew, Sew, Sew).asUInt(),
  ...
  object VimacType {
    ...
    def vmul = LiteralCat(
      ...
      FMT.VVV,
      VimacOpcode.vmul
    ) // "b00_110_0_000".U(OpTypeWidth.W) // vmul    vmul.vv/vmul.vx
    def vwmul = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.S,
      INT.X,
      FMT.VVW,
      VimacOpcode.vmul
    ) // "b00_110_1_000".U(OpTypeWidth.W) // vmul    vwmul.vv/vwmul.vx
    def vwmulu = LiteralCat(
      0.U(2.W),
      INT.U,
      INT.U,
      INT.X,
      FMT.VVW,
      VimacOpcode.vmul
    ) // "b00_000_1_000".U(OpTypeWidth.W) // vmul    vwmulu.vv/vwmulu.vx
    def vwmulsu = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.U,
      INT.X,
      FMT.VVW,
      VimacOpcode.vmul
    ) // "b00_100_1_000".U(OpTypeWidth.W) // vmul    vwmulsu.vv/vwmulsu.vx
    def vmulh = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.S,
      INT.X,
      FMT.VVV,
      VimacOpcode.vmulh
    ) // "b00_110_0_001".U(OpTypeWidth.W) // vmulh   vmulh.vv/vmulh.vx
    def vmulhu = LiteralCat(
      0.U(2.W),
      INT.U,
      INT.U,
      INT.X,
      FMT.VVV,
      VimacOpcode.vmulh
    ) // "b00_000_0_001".U(OpTypeWidth.W) // vmulh   vmulhu.vv/vmulhu.vx
    def vmulhsu = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.U,
      INT.X,
      FMT.VVV,
      VimacOpcode.vmulh
    ) // "b00_100_0_001".U(OpTypeWidth.W) // vmulh   vmulhsu.vv/vmulhsu.vx
    def vmacc = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.S,
      INT.S,
      FMT.VVV,
      VimacOpcode.vmacc
    ) // "b00_111_0_010".U(OpTypeWidth.W) // vmacc   vmacc.vv/vmacc.vx
    def vwmaccu = LiteralCat(
      0.U(2.W),
      INT.U,
      INT.U,
      INT.U,
      FMT.VVW,
      VimacOpcode.vmacc
    ) // "b00_000_1_010".U(OpTypeWidth.W) // vmacc   vwmaccu.vv/vwmaccu.vx
    def vwmacc = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.S,
      INT.S,
      FMT.VVW,
      VimacOpcode.vmacc
    ) // "b00_111_1_010".U(OpTypeWidth.W) // vmacc   vwmacc.vv/vwmacc.vx
    def vwmaccsu = LiteralCat(
      0.U(2.W),
      INT.U,
      INT.S,
      INT.S,
      FMT.VVW,
      VimacOpcode.vmacc
    ) // "b00_011_1_010".U(OpTypeWidth.W) // vmacc   vwmaccsu.vv/vwmaccsu.vx
    def vwmaccus = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.U,
      INT.S,
      FMT.VVW,
      VimacOpcode.vmacc
    ) // "b00_101_1_010".U(OpTypeWidth.W) // vmacc   vwmaccus.vx
    def vnmsac = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.S,
      INT.S,
      FMT.VVV,
      VimacOpcode.vnmsac
    ) // "b00_111_0_011".U(OpTypeWidth.W) // vnmsac  vnmsac.vv/vnmsac.vx
    def vmadd = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.S,
      INT.S,
      FMT.VVV,
      VimacOpcode.vmadd
    ) // "b00_111_0_100".U(OpTypeWidth.W) // vmadd   vmadd.vv/vmadd.vx
    def vnmsub = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.S,
      INT.S,
      FMT.VVV,
      VimacOpcode.vnmsub
    ) // "b00_111_0_101".U(OpTypeWidth.W) // vnmsub  vnmsub.vv/vnmsub.vx
    def vsmul = LiteralCat(
      0.U(2.W),
      INT.S,
      INT.S,
      INT.X,
      FMT.VVV,
      VimacOpcode.vsmul
    ) // "b00_110_0_110".U(OpTypeWidth.W) // vsmul   vsmul.vv/vsmul.vx
