...
class FloatCompare() extends Module {
  ...
  val isCmp = FcmpOpCode.isCmp(io.opCode)
  val isFeq = FcmpOpCode.isFeq(io.opCode)
  val isFlt = FcmpOpCode.isFlt(io.opCode)
  val isFle = FcmpOpCode.isFle(io.opCode)
  val isCmpNQ = FcmpOpCode.isCmpNQ(io.opCode)
  val isMaxMin = FcmpOpCode.isMaxMin(io.opCode)
  val isFmax = FcmpOpCode.isFmax(io.opCode)
  val isFmin = FcmpOpCode.isFmin(io.opCode)
  val isMaxMinM = FcmpOpCode.isMaxMinM(io.opCode)
  val isFsgn = FcmpOpCode.isFsgn(io.opCode)
  val isFsgnj = FcmpOpCode.isFsgnj(io.opCode)
  val isFsgnjx = FcmpOpCode.isFsgnjx(io.opCode)
  val isFsgnjn = FcmpOpCode.isFsgnjn(io.opCode)
  ...
  val resultCmpNormal = Mux1H(Seq(isFeq, isFlt, isFle), Seq(resultNormalEq, resultNormalLt, resultNormalLe))
...
object GenFloatCompare extends App {
  (new ChiselStage).execute(
    args,
    Seq(
      ChiselGeneratorAnnotation(() => new FloatCompare),
