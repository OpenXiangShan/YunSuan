...
class VFixPoint64b extends Module {
  ...
  def Adder_chain_rnd(data: Seq[UInt], rndInc: Seq[Bool]): Seq[UInt] = {
    ...
    for (i <- 0 until 8) {
      ...
      cin(i) := Mux1H(
        Mux(io.isNClip, (sew.oneHot.asUInt << 1)(3, 0), sew.oneHot.asUInt),
        Seq(1, 2, 4, 8).map(n => if ((i % n) == 0) rndInc(i) else cout(i - 1))
      )
  ...
  for (i <- 0 until 8) {
    high_avgBeforeRnd(i) := Mux1H(
      sew.oneHot,
      Seq(1, 2, 4, 8).map(n => if ((i % n) == (n - 1)) highBitAvg(i) else adderOut(i + 1)(0))
    )
  }
  ...
  io.vxsat := Mux(io.isNClip, nclipSat, Mux(opcode.isSatAdd, saturate, 0.U))
