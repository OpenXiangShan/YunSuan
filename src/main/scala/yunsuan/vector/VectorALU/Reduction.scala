...
class Reduction extends Module {
  ...
  vd_logical_alu := 0.U
  when(vredand_vs) {
    vd_logical_alu := vs12m_bits(2 * VLEN - 1, VLEN) & vs12m_bits(VLEN - 1, 0)
  }.elsewhen(vredor_vs) {
    vd_logical_alu := vs12m_bits(2 * VLEN - 1, VLEN) | vs12m_bits(VLEN - 1, 0)
  }.elsewhen(vredxor_vs) {
    vd_logical_alu := vs12m_bits(2 * VLEN - 1, VLEN) ^ vs12m_bits(VLEN - 1, 0)
  ...
  // Widen
  val vs_hi_widen = Mux1H(eewVs2.oneHot(2, 0), Seq(8, 16, 32).map(sew =>
    Cat(UIntSplit(vs12m_bits(127, 64), sew).map(BitsExtend(_, 2 * sew, signed)).reverse)))
  val vs_lo_widen = Mux1H(eewVs2.oneHot(2, 0), Seq(8, 16, 32).map(sew =>
    Cat(UIntSplit(vs12m_bits(63, 0), sew).map(BitsExtend(_, 2 * sew, signed)).reverse)))
  hi_vs := Mux(widen_reduction_uop, vs_hi_widen, vs12m_bits(255, 128))
  lo_vs := Mux(widen_reduction_uop, vs_lo_widen, vs12m_bits(127, 0))
  ...
  val old_vd_reg = RegEnable(Mux(alu_uop || widen_alu_uop, lo_vs, old_vd), 0.U, fire)
  ...
  val isVredmax_reg = RegEnable(opcode.isVredmax, false.B, fire)
  val isVredmin_reg = RegEnable(opcode.isVredmin, false.B, fire)
  ...
  val alu_uop_reg = RegEnable(alu_uop, false.B, fire)
  val widen_alu_uop_reg = RegEnable(widen_alu_uop, false.B, fire)
  val vdType_reg = RegEnable(vdType(1,0), 0.U, fire)
  ...
  csa_3to2_sew64.io.in_a := vs12m_bits_sew64(63, 0)
  csa_3to2_sew64.io.in_b := vs12m_bits_sew64(127, 64)
  ...
  csa_3to2_sew32_0.io.in_a := vs12m_bits_sew32(31, 0)
  csa_3to2_sew32_0.io.in_b := vs12m_bits_sew32(63, 32)
  csa_3to2_sew32_0.io.in_c := vs12m_bits_sew32(95, 64)
  sum_sew32(0) := csa_3to2_sew32_0.io.out_sum
  carry_sew32(0) := csa_3to2_sew32_0.io.out_car
  sum_add_sew32 := vs12m_bits_sew32(127, 96) + vs1_zero(31, 0)
  ...
  val in0_sew16 = Cat(vs1_zero(15, 0), vs12m_bits_sew16(127, 0))
  ...
  val in0_sew8 = vs12m_bits_sew8(127, 0)
  ...
  compare_3to1_sew64.io.a := vs12m_bits_sew64(63, 0)
  compare_3to1_sew64.io.b := vs12m_bits_sew64(127, 64)
  compare_3to1_sew64.io.c := vs1_zero
  compare_3to1_sew64.io.max := opcode.isVredmax
  ...
  compare_3to1_sew32.io.a := vs12m_bits_sew32(31, 0)
  compare_3to1_sew32.io.b := vs12m_bits_sew32(63, 32)
  compare_3to1_sew32.io.c := vs12m_bits_sew32(95, 64)
  compare_3to1_sew32.io.max := opcode.isVredmax
  ...
  compare0_2to1_sew32.io.a := vs12m_bits_sew32(127, 96)
  compare0_2to1_sew32.io.b := vs1_zero(31, 0)
  compare0_2to1_sew32.io.max := opcode.isVredmax
  ...
  val in0_max_sew16 = Cat(vs1_zero(15, 0), vs12m_bits_sew16(127, 0))
  ...
  for (i <- 0 until 3) {
    ...
    compare_3to1_sew16.io.max := opcode.isVredmax
  ...
  for (i <- 0 until 5) {
    ...
    compare_3to1_sew8.io.a := vs12m_bits_sew8(24 * i + 7, 24 * i + 0)
    compare_3to1_sew8.io.b := vs12m_bits_sew8(24 * i + 15, 24 * i + 8)
    compare_3to1_sew8.io.c := vs12m_bits_sew8(24 * i + 23, 24 * i + 16)
    compare_3to1_sew8.io.max := opcode.isVredmax
  ...
  compare0_2to1_sew8.io.a := vs12m_bits_sew8(127, 120)
  compare0_2to1_sew8.io.b := vs1_zero(7, 0)
  compare0_2to1_sew8.io.max := opcode.isVredmax
  ...
  val logical_vd = Mux(alu_uop, vd_logical_alu, Cat(0.U((VLEN - 64).W), vd_logical(~vdType(1, 0))))
