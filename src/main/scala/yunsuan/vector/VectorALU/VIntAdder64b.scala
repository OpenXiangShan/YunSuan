...
class VIntAdder64b extends Module {
  ...
  val vs2Widen = Mux1H(
    eewVs1.oneHot.take(3),
    Seq(8, 16, 32).map(sew => Cat(UIntSplit(vs2_32b, sew).map(BitsExtend(_, 2 * sew, signed)).reverse))
  )
  val vs1_32b = Mux(uopIdx(0), vs1(63, 32), vs1(31, 0))
  val vs1Widen = Mux1H(
    eewVs1.oneHot.take(3),
    Seq(8, 16, 32).map(sew => Cat(UIntSplit(vs1_32b, sew).map(BitsExtend(_, 2 * sew, signed)).reverse))
  )
  ...
  val cmpEq = Mux1H(
    Seq(
      eewVs1.is8 -> equal,
      eewVs1.is16 -> Cat(
        Fill(2, equal(7, 6).andR),
        Fill(2, equal(5, 4).andR),
        Fill(2, equal(3, 2).andR),
        Fill(2, equal(1, 0).andR)
      ),
