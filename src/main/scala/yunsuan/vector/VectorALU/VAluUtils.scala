...
object MaskExtract {
  def apply(vmask128b: UInt, uopIdx: UInt, sew: SewOH) = {
    ...
    extracted := Mux1H(
      Seq.tabulate(8)(uopIdx === _.U),
      Seq.tabulate(8)(idx =>
        Mux1H(sew.oneHot, Seq(16, 8, 4, 2).map(stride => vmask128b((idx + 1) * stride - 1, idx * stride)))
      )
    )
    extracted
  }
...
object UIntToCont0s {
  def apply(data: UInt, dw: Int): UInt = { // dw is width of data
    if (dw == 1) {
      ...
      Mux(
        data(dw - 1),
        Cat(apply(data(dw - 2, 0), dw - 1), 0.U((1 << (dw - 1)).W)),
...
object TailGen {
  def apply(vl: UInt, uopIdx: UInt, eew: SewOH, narrow: Bool = false.B): UInt = {
    ...
    // vl - uopIdx * 128/eew
    val nElemRemain = Cat(0.U(1.W), vl) - Mux1H(
      eew.oneHot,
      Seq(4, 3, 2, 1).map(x => Cat(Mux(narrow, uopIdx(2, 1), uopIdx(2, 0)), 0.U(x.W)))
    )
...
object PrestartGen {
  def apply(vstart: UInt, uopIdx: UInt, eew: SewOH, narrow: Bool = false.B): UInt = {
    ...
    val nElemRemain = Cat(0.U(1.W), vstart) - Mux1H(
      eew.oneHot,
      Seq(4, 3, 2, 1).map(x => Cat(Mux(narrow, uopIdx(2, 1), uopIdx(2, 0)), 0.U(x.W)))
    )
    val maxNElemInOneUop = Mux1H(eew.oneHot, Seq(16.U, 8.U, 4.U, 2.U))
