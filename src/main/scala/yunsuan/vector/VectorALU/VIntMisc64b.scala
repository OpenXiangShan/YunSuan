...
class VIntMisc64b extends Module {
  ...
  when(vf2) { // sew = 16/32/64
    extResult := Mux(
      eewVd.is16,
      BitsExtend.vector(Mux1H(expdIdxOH_vf2.take(2), UIntSplit(vs2, 32)), 64, extSign, 8),
      Mux(
        eewVd.is32,
        BitsExtend.vector(Mux1H(expdIdxOH_vf2.take(2), UIntSplit(vs2, 32)), 64, extSign, 16),
    ...
    extResult := Mux(
      eewVd.is32,
      BitsExtend.vector(Mux1H(expdIdxOH_vf4.take(4), UIntSplit(vs2, 16)), 64, extSign, 8),
  ...
  val vs1_revsByElem = MuxCase(
    vs1,
    Seq( // reverse vs1 by element when left-shift
  ...
  when(Mux(narrow, eewVd.is16, eewVd.is32)) {
    ...
    io.toFixP.rnd_high := Cat(
      Fill(2, shift16(3)._2),
      Fill(2, shift16(2)._2),
      Fill(2, shift16(1)._2),
      Fill(2, shift16(0)._2)
    )
    io.toFixP.rnd_tail := Cat(
      Fill(2, shift16(3)._3),
      Fill(2, shift16(2)._3),
      Fill(2, shift16(1)._3),
      Fill(2, shift16(0)._3)
  ...
  val vmask_adjust =
    Mux1H(eewVd.oneHot, Seq(1, 2, 4, 8).map(k => Cat(Seq.tabulate(8 / k)(i => Fill(k, io.vmask(i))).reverse)))
  ...
  io.vd := Mux(
    opcode.isShift,
    shiftResult,
    Mux(opcode.isVext, extResult, Mux(opcode.isBitLogical, bitLogical, mergeMove))
  )
}
