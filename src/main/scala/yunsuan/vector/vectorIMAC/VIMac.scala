...
class VIMac extends Module {
  ...
  for (i <- 0 until 2) {
    ...
    vIMac64bs(i).io.vs1 := Mux(widen, Cat(UIntSplit(vs1, 32)(i + 2), UIntSplit(vs1, 32)(i)), UIntSplit(vs1, 64)(i))
    vIMac64bs(i).io.vs2 := Mux(
      opcode.overWriteMultiplicand,
      UIntSplit(oldVd, 64)(i),
      Mux(widen, Cat(UIntSplit(vs2, 32)(i + 2), UIntSplit(vs2, 32)(i)), UIntSplit(vs2, 64)(i))
    )
    vIMac64bs(i).io.oldVd := Mux(opcode.overWriteMultiplicand, UIntSplit(vs2, 64)(i), UIntSplit(oldVd, 64)(i))
  }
  ...
  val bitsReplace = Cat(
    updateType.zipWithIndex
      .map({
        case (x, i) =>
          Mux(!x(1), 0.U(8.W), Mux(x(0), ~0.U(8.W), UIntSplit(oldVdS2, 8)(i)))
      })
      .reverse
  )
