...
class VIMac64b_noBooth extends Module {
  ...
  partProd(0) := Mux(
    !vs1(0),
    0.U,
    Mux1H(sew.oneHot, Seq(8, 16, 32, 64).map(k => Cat(Fill(128 - k, vs2_is_signed && vs2(k - 1)), vs2(k - 1, 0))))
  )
  for (i <- 1 until 64) {
    partProd(i) := Mux(
      !vs1(i),
      ...
      Mux1H(
        sew.oneHot,
        Seq(8, 16, 32, 64).map(k =>
          Cat(
            Fill(128 - k - i - i / k * k, vs2_is_signed && vs2(i / k * k + k - 1)),
            vs2(i / k * k + k - 1, i / k * k),
            0.U((i + i / k * k).W)
          )
        )
  ...
  for (i <- 0 until 64) {
    ...
    if (eIdx8 == 0) { partProdSet0_sew8(i) := Cat(0.U(112.W), partProdReverse(i)(15, 0)) }
      ...
      partProdSet0_sew8(i) := Cat(
        0.U((112 - eIdx8 * 16).W),
        partProdReverse(i)(eIdx8 * 16 + 15, eIdx8 * 16),
        0.U((eIdx8 * 16).W)
      )
    }
    ...
    if (eIdx16 == 0) { partProdSet0_sew16(i) := Cat(0.U(96.W), partProdReverse(i)(31, 0)) }
      ...
      partProdSet0_sew16(i) := Cat(
        0.U((96 - eIdx16 * 32).W),
        partProdReverse(i)(eIdx16 * 32 + 31, eIdx16 * 32),
        0.U((eIdx16 * 32).W)
      )
    }
  ...
  for (i <- 0 until 64) {
    if (i % 8 != 0 | i == 0) {
      ...
      partProdFinal(i) := Mux(
        vs1(i - 1) && vs1_is_signed && !sew.is64,
        partProdSet0(i) |
          Mux1H(sew.oneHot.take(3), Seq(8, 16, 32).map(n => 1.U << (2 * i - 2 * n))),
        partProdSet0(i)
      )
    } else if (i % 16 == 0) {
      partProdFinal(i) := Mux(
        vs1(i - 1) && vs1_is_signed && (sew.is16 || sew.is8),
        partProdSet0(i) |
          Mux1H(sew.oneHot.take(2), Seq(8, 16).map(n => 1.U << (2 * i - 2 * n))),
        partProdSet0(i)
      )
    } else {
      partProdFinal(i) := Mux(
        vs1(i - 1) && vs1_is_signed && sew.is8,
        partProdSet0(i) | 1.U << (2 * i - 16),
        partProdSet0(i)
      )
    }
  }
  partProdFinal(64) := Mux(
    vs1(63) && vs1_is_signed,
    Mux1H(sew.oneHot, Seq(8, 16, 32, 64).map(n => 1.U << (128 - 2 * n))),
    0.U
  )
  ...
  val oldVdReorg = Mux1H(
    sew.oneHot,
    Seq(8, 16, 32, 64).map(sew => VecInit(UIntSplit(oldVd, sew).map(x => BitsExtend(x, 2 * sew, false.B))).asUInt)
  )
  partProdFinal(65) := Mux(io.isMacc, Mux(io.widen, Cat(oldVd, oldVd), Mux(io.isSub, ~oldVdReorg, oldVdReorg)), 0.U)
  partProdFinal(66) := Mux(
    io.isSub,
    Mux1H(sew.oneHot, Seq(8, 16, 32, 64).map(sew => Fill(64 / sew, 1.U((2 * sew).W)))),
    0.U
  )
  // 3-bit full-adder
  ...
  def reduce3to2(data: Seq[UInt]): Seq[UInt] = {
    ...
    for (i <- 0 until n3Group) {
      cout(i) := add3_UInt(data(3 * i), data(3 * i + 1), data(3 * i + 2))._1 &
        Mux1H(
          Seq(
            sew.is8 -> "h7fff7fff7fff7fff7fff7fff7fff7fff".U(128.W),
  ...
  for (i <- 0 until 8) {
    val adder_16b =
      new Adder_16b(wallaceOutReg(0)(16 * i + 15, 16 * i), wallaceOutReg(1)(16 * i + 15, 16 * i), cin_wo(i))
  ...
  val vdS2 = PriorityMux(
    Seq(
      (sewS2.is64 || widenS2) -> Mux(
        sewS2.is64 && highHalfS2 || widenS2 && uopIdxS2(0),
        walOut(127, 64),
        walOut(63, 0)
      ),
