 ...
 class VectorIntAdder() extends Module {
   ...
   val is_add = io.op_code === VipuType.vadd_vv  //vadd
   val is_sub = io.op_code === VipuType.vsub_vv || io.op_code === VipuType.vrsub_vv  //vsub vrsub
   val is_add_carry = io.op_code === VipuType.vadc_vvm || io.op_code === VipuType.vmadc_vvm || io.op_code === VipuType.vmadc_vv  //vadc vmadc
   val is_sub_borrow = io.op_code === VipuType.vsbc_vvm || io.op_code === VipuType.vmsbc_vvm || io.op_code === VipuType.vmsbc_vv  //vsbc vmsbc
   val is_unsigned_widening_add = io.op_code === VipuType.vwaddu_vv  //vwaddu
   val is_unsigned_widening_sub = io.op_code === VipuType.vwsubu_vv  //vwsubu
   val is_signed_widening_add = io.op_code === VipuType.vwadd_vv  //vwadd
   val is_signed_widening_sub = io.op_code === VipuType.vwsub_vv  //vwsub
   val is_unsigned_widening_add_in0widening = io.op_code === VipuType.vwaddu_wv  //vwaddu
   val is_unsigned_widening_sub_in0widening = io.op_code === VipuType.vwaddu_wv  //vwsubu
   val is_signed_widening_add_in0widening = io.op_code === VipuType.vwadd_wv  //vwadd
   val is_signed_widening_sub_in0widening = io.op_code === VipuType.vwsub_wv  //vwsub
   val is_max_unsigned = io.op_code === VipuType.vmaxu_vv  //vmaxu
   val is_min_unsigned = io.op_code === VipuType.vminu_vv  //vminu
   val is_max_signed = io.op_code === VipuType.vmax_vv  //vmax
   val is_min_signed = io.op_code === VipuType.vmin_vv  //vmin
   val is_equal = io.op_code === VipuType.vmseq_vv  //vmseq
   val is_not_equal = io.op_code === VipuType.vmsne_vv  //vmsne
   val is_less_than_unsigned = io.op_code === VipuType.vmsltu_vv  //vmsltu
   val is_less_than_signed = io.op_code === VipuType.vmslt_vv  //vmslt
   val is_less_than_or_equal_unsigned = io.op_code === VipuType.vmsleu_vv  //vmsleu
   val is_less_than_or_equal_signed = io.op_code === VipuType.vmsle_vv  //vmsle
   val is_greater_than_unsigned = io.op_code === VipuType.vmsgtu_vv  //vmsgtu
   val is_greater_than_signed = io.op_code === VipuType.vmsgt_vv  //vmsgt
   val is_greater_than_or_equal_unsigned = false.B // io.op_code === VipuType.greaterThanOrEqualUnsigned  //vmsgeu
   val is_greater_than_or_equal_signed = false.B // io.op_code === VipuType.greaterThanOrEqualSigned  //vmsge
   val is_bitwise_logical_and = io.op_code === VipuType.vmand_mm //vand
   val is_bitwise_logical_nand = io.op_code === VipuType.vmnand_mm  //vnand
   val is_bitwise_logical_andn = io.op_code === VipuType.vmandn_mm  //vandn
   val is_bitwise_logical_or = io.op_code === VipuType.vmor_mm  //vor
   val is_bitwise_logical_nor = io.op_code === VipuType.vmnor_mm  //vnor
   val is_bitwise_logical_orn = io.op_code === VipuType.vmorn_mm  //vorn
   val is_bitwise_logical_xor = io.op_code === VipuType.vmxor_mm  //vxor
   val is_bitwise_logical_xnor = io.op_code === VipuType.vmxnor_mm  //vxnor
   val is_shift_left_logical = io.op_code === VipuType.vsll_vv  //vsll
   val is_shift_right_logical = io.op_code === VipuType.vsrl_vv  //vsrl
   val is_shift_right_arithmetic = io.op_code === VipuType.vsra_vv  //vsra
   val is_scaling_shift_right_logical = io.op_code === VipuType.vssrl_vv  //vssrl
   val is_scaling_shift_right_arithmetic =io.op_code === VipuType.vssra_vv  //vssra
