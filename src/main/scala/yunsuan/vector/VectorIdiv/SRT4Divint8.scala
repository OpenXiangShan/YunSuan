...
class SRT4Divint8(bit_width: Int = 8) extends Module {
  ...
  val w_init =
    Wire(
      Vec(2, UInt((w_width + 2).W))
    ) // because we donot take bypass to reduce the size，after each iteration w*4，so we need to rshift lzc_d + 2 + ralign，to not rshift 2 bit we extend the result of each iter as not mul 4 wich need extra 2bit for shift
  ...
  val early_q = Mux(x_small, 0.U, Mux(d_zero, Fill(bit_width, 1.U), 0.U))
  ...
  w_init(0) := Mux(
    early_finish,
    early_rem,
    Cat(0.U(5.W), Mux(lzc_diff(0), Cat(0.U(1.W), norm_x), Cat(norm_x, 0.U(1.W))))
  )
  w_init(1) := 0.U
  ...
  init_q := Cat(
    0.U(2.W), //-2 -1 attentin the msb of cat opeartion is at the right，so it reverse :vec 0 -2d 1 -d 2 0 3 d 4 2d
...
//  w_iter_c := Mux(stateReg(pre),w_iter_c_init,IterBlock_SR4.io.nxt_w)
  ...
  val adjust =
    Mux(x_sign_reg, (~rem_is_zero).asBool & (~rem_sign(w_width - 1)).asBool, rem_sign(w_width - 1)) & !early_finish_q
  val out_q_final =
  ...
  val out_rem_final =
    Mux(
      early_finish_q,
      rem_adjust.asSInt,
      rem_adjust.asSInt >> (lzc_d_reg + 1.U)
    ) // attention arithmetic rshift， because it has been sign adjusted,(can put it in the post stage?)
