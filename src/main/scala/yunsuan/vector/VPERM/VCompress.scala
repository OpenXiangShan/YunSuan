...
class Compress(n: Int) extends VPermModule {
  ...
  for (i <- 0 until n) {
    ...
    val compress_en = mask_reg(i).asBool && (os_base_reg <= cmos_vec_reg(i)) && (cmos_vec_reg(
      i
    ) < (os_base_reg +& elem_vld_reg)) && (cmos_vec_reg(i) < vl_reg)

    compressed_en_vec(i) := ZeroExt(compress_en.asUInt << res_idx, n)
    compressed_data_vec(i) := Fill(VLEN, compress_en.asUInt) & ZeroExt(
      (src_data_vec(i) << (res_idx << log2Up(VLEN / n))),
      VLEN
    )
  }
...
class CompressModule extends VPermModule {
  ...
  when(io.uop_idx === 42.U) {
    ...
    (io.uop_idx === 12.U) || (io.uop_idx === 17.U) || (io.uop_idx === 23.U) || (io.uop_idx === 30.U) || (io.uop_idx === 38.U)
  ) {
    vd_idx := 3.U
  }.elsewhen(
    (io.uop_idx === 7.U) || (io.uop_idx === 11.U) || (io.uop_idx === 16.U) || (io.uop_idx === 22.U) || (io.uop_idx === 29.U) || (io.uop_idx === 37.U)
  ) {
    vd_idx := 2.U
  }.elsewhen(
    (io.uop_idx === 3.U) || (io.uop_idx === 6.U) || (io.uop_idx === 10.U) || (io.uop_idx === 15.U) || (io.uop_idx === 21.U) || (io.uop_idx === 28.U) || (io.uop_idx === 36.U)
  ) {
    vd_idx := 1.U
  }.otherwise {
    vd_idx := 0.U
  ...
  val compress_module = VecInit(
    Seq(compress_module_0.io, compress_module_1.io, compress_module_2.io, compress_module_3.io)
  )
  for (i <- 0 until 4) {
  ...
  val compress_res_data = LookupTree(
    vformat,
    List(
  ...
  val ones_sum_res_data = LookupTree(
    vformat,
    List(
