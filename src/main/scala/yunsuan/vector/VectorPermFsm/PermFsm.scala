...
class PermFsm extends Module {
  ...
  val vs1_type = Mux(vrgather16, 1.U, vsew_reg)
  ...
  // val update_vs_idx = Mux(reg_vrgather, !Cat(table_hi_lo.reverse).orR && (perm_state === rd_vs), perm_state === rd_vs)
  ...
  val old_vd_idx = Mux(reg_vcompress, rd_vd_idx, Mux(vs_idx === vlmul_reg, vs_idx, vs_idx + 1.U))
  ...
  when((rd_vd_idx === vlmul_reg) && cmprs_rd_vd) {
    rd_vd_idx := 0.U
  ...
  def access_table_gen(vs1: UInt, vsew: UInt): Seq[UInt] = {
    val sew = SewOH(vsew)
  ...
  when(viq01_valid) {
    access_table(viq0_uop_idx) := access_table_gen(viq0_vs1, viq0_sew)
    access_table(viq1_uop_idx) := access_table_gen(viq1_vs1, viq1_sew)
  }.elsewhen(viq0_valid) {
    access_table(viq0_uop_idx) := access_table_gen(viq0_vs1, viq0_sew)
  }.elsewhen(viq1_valid) {
    access_table(viq1_uop_idx) := access_table_gen(viq1_vs1, viq1_sew)
  ...
  //  }.elsewhen(viq1_valid && (viq1_uop_idx === 0.U)) {
  //    old_vd := viq1_old_vd
  //  }.elsewhen(!reg_vcompress && update_vs_idx_reg(4)) {
  //    old_vd := io.in.fsm_rd_data_3
  //  }.elsewhen(reg_vcompress && cmprs_rd_vd_reg(4)) {
  //    old_vd := io.in.fsm_rd_data_3
  ...
  when(update_table_reg(4) && update_vs_idx_reg(4)) {
  ...
  //   one_sum := 0.U
  // }.elsewhen(update_vs_idx_reg(4)) {
  //   when((one_sum + current_vs_ones_sum) >= Cat(vd_idx_plus1, 0.U(4.W))) {
  //     one_sum := one_sum + current_vs_ones_sum - vlenb.U
  //   }.otherwise {
  //     one_sum := one_sum + current_vs_ones_sum
  //   }
  // }.elsewhen(one_sum >= Cat(vd_idx_plus1, 0.U(4.W))) {
  //   one_sum := one_sum - vlenb.U
  // }

  // when(calc_done) {
  when(cmprs_rd_vd_reg(4)) {
    one_sum := 0.U
  }.elsewhen(update_vs_idx_reg(4)) {
    one_sum := one_sum + current_vs_ones_sum
  }
  ...
  io.out.fsm_wb_vld := Mux(reg_vcompress, cmprs_wb_vld, update_vs_idx_reg(0))
  io.out.fsm_wb_preg_idx := vd_preg_idx(vs_idx_reg)
