...
import yunsuan.{VectorElementFormat, VfaddOpCode}
...
class VectorFloatAdder() extends Module {
  ...
  val is_fmove =
    io.op_code === VfaddOpCode.fmove || (io.op_code === VfaddOpCode.fmv_f_s) || (io.op_code === VfaddOpCode.fmv_s_f)
  ...
  val U_F32_Mixed_0 = Module(
    new FloatAdderF32WidenF16MixedPipeline(is_print = false, hasMinMaxCompare = hasMinMaxCompare)
  )
  ...
  val fp_f16_result = Cat(
    Fill(16, is_vec_reg) & U_F16_3_result,
    Fill(16, is_vec_reg) & U_F16_2_result,
    Fill(16, is_vec_reg) & U_F16_1_result,
    U_F16_0_result
  )
...
private[vector] class FloatAdderWidenFormat() extends Module {
  ...
  def Widen(in: UInt): UInt = {
    ...
    if (in.getWidth == 16) {
      val U_fp_a_is_denormal_to_widen = Module(
        new ShiftLeftPriorityWithF32EXPResult(srcW = sig_w - 1, priorityShiftValueW = sig_w - 1, expW = dest_exp_w)
      )
      ...
      val U_fp_a_is_denormal_to_widen = Module(
        new ShiftLeftPriorityWithF64EXPResult(srcW = sig_w - 1, priorityShiftValueW = sig_w - 1, expW = dest_exp_w)
      )
      U_fp_a_is_denormal_to_widen.io.src := in(sig_w - 2, 0)
      U_fp_a_is_denormal_to_widen.io.priority_shiftValue := in(sig_w - 2, 0)
      fp_a_lshift := U_fp_a_is_denormal_to_widen.io.lshift_result
  ...
  io.widen_b_f32_0 := Mux(
    io.is_frs1,
    Widen(io.frs1(15, 0)),
    Mux(io.uop_idx, Widen(io.widen_b(47, 32)), Widen(io.widen_b(15, 0)))
  )
  io.widen_a_f32_1 := Mux(io.uop_idx, Widen(io.widen_a(63, 48)), Widen(io.widen_a(31, 16)))
  io.widen_b_f32_1 := Mux(
    io.is_frs1,
    Widen(io.frs1(15, 0)),
    Mux(io.uop_idx, Widen(io.widen_b(63, 48)), Widen(io.widen_b(31, 16)))
  )
  io.widen_a_f64 := Mux(io.uop_idx, Widen(io.widen_a(63, 32)), Widen(io.widen_a(31, 0)))
  io.widen_b_f64 := Mux(
    io.is_frs1,
    Widen(io.frs1(31, 0)),
    Mux(io.uop_idx, Widen(io.widen_b(63, 32)), Widen(io.widen_b(31, 0)))
  )
}

private[vector] class FloatAdderF32WidenF16MixedPipeline(
...
//  )
  val fp_a_to32 = Mux(io.res_widening, io.widen_a, Mux(res_is_f32, io.fp_a, fp_a_16as32))
...
//  )
  val fp_b_to32 = Mux(io.res_widening & !io.opb_widening, io.widen_b, Mux(res_is_f32, io.fp_b, fp_b_16as32))
  ...
  val U_close_path = Module(
    new ClosePathF32WidenF16MixedPipeline(is_print = is_print, hasMinMaxCompare = hasMinMaxCompare)
  )
  ...
  val fp_a_is_SNAN =
    io.fp_aIsFpCanonicalNAN | Efp_a_is_all_one & fp_a_mantissa_isnot_zero & !fp_a_to32(significandWidth - 2)
  val fp_b_is_NAN = io.fp_bIsFpCanonicalNAN | Efp_b_is_all_one & fp_b_mantissa_isnot_zero
  val fp_b_is_SNAN =
    io.fp_bIsFpCanonicalNAN | Efp_b_is_all_one & fp_b_mantissa_isnot_zero & !fp_b_to32(significandWidth - 2)
  ...
  val is_far_path =
    !EOP | (EOP & absEaSubEb(absEaSubEb.getWidth - 1, 1).orR) | (absEaSubEb === 1.U & (Efp_a_is_zero ^ Efp_b_is_zero))
  ...
  when(RegNext((fp_a_is_SNAN | fp_b_is_SNAN) | (EOP & fp_a_is_infinite & fp_b_is_infinite))) {
    ...
    RegNext(
      fp_a_is_NAN | fp_b_is_NAN | fp_a_is_infinite | fp_b_is_infinite | ((fp_b_is_zero | fp_a_is_zero) & io.res_widening)
    )
  ) {
  ...
  val out_NAN_reg =
    Mux(RegNext(res_is_f32), Cat(0.U, Fill(8, 1.U), 1.U, 0.U(22.W)), Cat(0.U(17.W), Fill(5, 1.U), 1.U, 0.U(9.W)))
  val out_infinite_sign = Mux(fp_a_is_infinite, fp_a_to32.head(1), io.is_sub ^ fp_b_to32.head(1))
  val out_infinite_reg = Mux(
    RegNext(res_is_f32),
    Cat(RegNext(out_infinite_sign), Fill(8, 1.U), 0.U(23.W)),
    Cat(0.U(16.W), RegNext(out_infinite_sign), Fill(5, 1.U), 0.U(10.W))
  )
  val out_fp32_reg = Mux(RegNext(is_far_path), U_far_path.io.fp_c, U_close_path.io.fp_c)
  val out_fp32_to_fp16_or_fp32_reg =
    Mux(RegNext(res_is_f32), out_fp32_reg, Cat(0.U(16.W), out_fp32_reg(31), out_fp32_reg(27, 23), out_fp32_reg(22, 13)))
  when(RegNext(fp_a_is_NAN | fp_b_is_NAN | (EOP & fp_a_is_infinite & fp_b_is_infinite))) {
    ...
    float_adder_result := Cat(
      RegNext(Mux(io.round_mode === "b010".U & EOP | (fp_a_to32.head(1).asBool & !EOP), 1.U, 0.U)),
      0.U(31.W)
    )
  ...
  if (hasMinMaxCompare) {
    ...
    val fp_b_is_greater =
      (!fp_b_sign & ((fp_a_sign & !(fp_b_is_zero & fp_a_is_zero)) | fp_b_exponent_is_greater | (fp_b_exponent_is_equal & fp_b_significand_is_greater))) |
        (fp_b_sign & fp_a_sign & (fp_b_exponent_is_smaller | (fp_b_exponent_is_equal & fp_b_significand_is_smaller)))
    val fp_b_is_equal =
      (fp_b_sign_is_equal & fp_b_exponent_is_equal & fp_b_significand_is_equal) | (fp_b_is_zero & fp_a_is_zero)
    ...
    val in_NAN =
      Mux(res_is_f32, Cat(0.U(1.W), Fill(9, 1.U(1.W)), 0.U(22.W)), Cat(0.U(17.W), Fill(6, 1.U(1.W)), 0.U(9.W)))
    val fp_aFix = Mux(io.fp_aIsFpCanonicalNAN, in_NAN, io.fp_a)
    val fp_bFix = Mux(io.fp_bIsFpCanonicalNAN, in_NAN, io.fp_b)
    val result_fsgnj =
      Mux(res_is_f32, Cat(fp_bFix.head(1), fp_aFix(30, 0)), Cat(0.U(16.W), Cat(fp_bFix(15), fp_aFix(14, 0))))
    val result_fsgnjn =
      Mux(res_is_f32, Cat(~fp_bFix.head(1), fp_aFix(30, 0)), Cat(0.U(16.W), Cat(~fp_bFix(15), fp_aFix(14, 0))))
    ...
    result_max := Mux1H(
      ...
      Seq(
        Mux(
          fp_b_is_greater.asBool || (!fp_b_sign.asBool && fp_b_is_zero && fp_a_is_zero),
          fp_b_16_or_32,
          fp_a_16_or_32
        ),
        fp_a_16_or_32,
...
private[this] class ShiftLeftPriorityWithF32EXPResult(val srcW: Int, priorityShiftValueW: Int, expW: Int)
    extends Module {
  val io = IO(new Bundle() {
...
private[this] class FarPathF32WidenF16MixedAdderPipeline(
  val AW:               Int,
  val AdderType:        String,
  val stage0AdderWidth: Int = 0)
    extends Module {
  val io = IO(new Bundle() {
  ...
  if (stage0AdderWidth == 0) io.result := RegNext(io.A) + RegNext(io.B)
    ...
    val seq_short_full_adder = (for (i <- 0 until short_adder_num) yield {
      if (i == (short_adder_num - 1)) io.A(AW - 1, stage0AdderWidth * i) + io.B(AW - 1, stage0AdderWidth * i)
        ...
        io.A(stage0AdderWidth * (i + 1) - 1, stage0AdderWidth * i) +& io
          .B(stage0AdderWidth * (i + 1) - 1, stage0AdderWidth * i)
    }).reverse.reduce(Cat(_, _))
    val reg_short_adder_sum = (for (i <- 0 until short_adder_num) yield {
      if (i == (short_adder_num - 1))
        seq_short_full_adder(seq_short_full_adder.getWidth - 1, (stage0AdderWidth + 1) * i)
    ...
    io.result := Cat(
      RegNext(reg_short_adder_sum(AW - 1, stage0AdderWidth)) + RegNext(
        Cat(reg_short_adder_car, 0.U)(AW - 1, stage0AdderWidth)
      ),
...
private[this] class FarPathF32WidenF16MixedPipeline(
  ...
  val fp_b_mantissa_widen =
    Mux(EOP, Cat(~significand_fp_b, 1.U, Fill(widenWidth, 1.U)), Cat(0.U, significand_fp_b, 0.U(widenWidth.W)))
  ...
  val fp_a_mantissa_widen =
    Mux(EOP, Cat(~significand_fp_a, 1.U, Fill(widenWidth, 1.U)), Cat(0.U, significand_fp_a, 0.U(widenWidth.W)))
  ...
  val B =
    Mux(absEaSubEb_is_greater, Fill(significandWidth + 1, EOP), far_rshift_widen_result.head(significandWidth + 1))
  val B_guard_normal = Mux(
    ...
    Mux(
      ...
      Mux(
        ...
        !far_rshift_widen_result.head(11 + 2)(0).asBool
      ),
      Mux(
        res_is_f32,
        far_rshift_widen_result.head(significandWidth + 2)(0).asBool,
  ...
  val B_round_normal = Mux(
    ...
    Mux(
      ...
      Mux(
        ...
        !far_rshift_widen_result.head(11 + 3)(0).asBool
      ),
      Mux(
        res_is_f32,
        far_rshift_widen_result.head(significandWidth + 3)(0).asBool,
  ...
  val B_sticky_normal = Mux(
    ...
    Mux(
      ...
      Mux(
        res_is_f32,
        (~far_rshift_widen_result.tail(significandWidth + 3)).asUInt.orR,
        (~far_rshift_widen_result.tail(11 + 3)).asUInt.orR
      ),
  ...
  val B_guard_overflow = Mux(
    ...
    Mux(
      ...
      Mux(
        res_is_f32,
        !far_rshift_widen_result.head(significandWidth + 1)(0).asBool,
        !far_rshift_widen_result.head(11 + 1)(0).asBool
      ),
      Mux(
        res_is_f32,
        far_rshift_widen_result.head(significandWidth + 1)(0).asBool,
  ...
  val U_FS0 = Module(
    new FarPathF32WidenF16MixedAdderPipeline(AW = A.getWidth, AdderType = "FS0", stage0AdderWidth = 18)
  )
  U_FS0.io.A := A
  U_FS0.io.B := B
  val FS0_reg = U_FS0.io.result
  val U_FS1 = Module(
    new FarPathF32WidenF16MixedAdderPipeline(AW = A.getWidth, AdderType = "FS1", stage0AdderWidth = 18)
  )
  ...
  val grs_overflow_reg = RegNext(
    Mux(
      EOP,
      Cat(A_0, 0.U, 0.U) - Cat(~B_0, B_guard_normal, B_rsticky_normal),
      Cat(A_0 ^ B_0, B_guard_normal, B_rsticky_normal)
    )
  )
  ...
  far_exponent_result_reg := Mux(
    far_case_overflow_reg | (FS1_reg.head(1).asBool & FS0_0_reg & far_case_normal_round_up_reg) | (RegNext(
      !EA.orR
    ) & FS0_reg.tail(1).head(1).asBool),
    RegNext(EA_add1),
    RegNext(EA)
  )
  OF_reg := RegNext(Mux(res_is_f32, EA_add1.andR, EA_add1.tail(3).andR)) & (far_case_overflow_reg | (FS1_reg
    .head(1)
    .asBool & FS0_0_reg & far_case_normal_round_up_reg))
  ...
  far_fraction_result_reg := Mux1H(
    ...
    Seq(
      Cat(
        FS0_reg(significandWidth - 2, 1 + 13),
        Mux(res_is_f32_reg, FS0_reg(13), FS0_reg(13) ^ far_case_normal_round_up_reg),
        FS0_reg(12, 1),
      ...
      Cat(
        FS0_reg(significandWidth - 1, 2 + 13),
        Mux(res_is_f32_reg, FS0_reg(14), FS0_reg(14) ^ far_case_overflow_round_up_reg),
        FS0_reg(13, 2),
...
private[this] class ClosePathF32WidenF16MixedPipeline(
  ...
  val mask_Efp_a_onehot = Cat(
    Efp_a === 0.U | Efp_a === 1.U,
    (for (i <- 2 until significandWidth + 1) yield (Efp_a === i.U).asUInt).reduce(Cat(_, _))
  )
  val mask_Efp_b_onehot = Cat(
    Efp_b === 0.U | Efp_b === 1.U,
    (for (i <- 2 until significandWidth + 1) yield (Efp_b === i.U).asUInt).reduce(Cat(_, _))
  )
  ...
  U_CS4.io.adder_op1 := RegNext(
    Mux(
      ...
      Cat(
        ...
        Mux(
          res_is_f32,
          significand_fp_a(significandWidth - 1, 1),
          Cat(significand_fp_a(significandWidth - 1, 1 + 13), 0.U(13.W))
        )
      ),
      Cat(
        0.U,
        Mux(
          res_is_f32,
          significand_fp_b(significandWidth - 1, 1),
          Cat(significand_fp_b(significandWidth - 1, 1 + 13), 0.U(13.W))
        )
      )
    )
  )
  ...
  val CS2_round_up = Mux(res_is_f32, significand_fp_b(0), significand_fp_b(0 + 13)) & (
    (RUP & !fp_a_sign) | (RDN & fp_a_sign) | (RNE & Mux(res_is_f32, CS2(1), CS2(1 + 13)) & Mux(
      res_is_f32,
      CS2(0),
      CS2(0 + 13)
    )) | RMM
  ...
  val CS3_round_up = Mux(res_is_f32, significand_fp_a(0), significand_fp_a(0 + 13)) & (
    (RUP & fp_a_sign) | (RDN & !fp_a_sign) | (RNE & Mux(res_is_f32, CS3(1), CS3(1 + 13)) & Mux(
      res_is_f32,
      CS3(0),
      CS3(0 + 13)
    )) | RMM
  ...
  val sel_CS2 = !exp_is_equal & !Efp_b_is_greater & ((!U_CS2.io.result
    .head(1)
    .asBool | Mux(res_is_f32, !U_CS2.io.result(0), !U_CS2.io.result(0 + 13)).asBool) | !CS2_round_up)
  val sel_CS3 = !exp_is_equal & Efp_b_is_greater & ((!U_CS3.io.result
    .head(1)
    .asBool | Mux(res_is_f32, !U_CS3.io.result(0), !U_CS3.io.result(0 + 13)).asBool) | !CS3_round_up)
  val sel_CS4 = !exp_is_equal & (
    (!Efp_b_is_greater & U_CS2.io.result.head(1).asBool & Mux(
      res_is_f32,
      U_CS2.io.result(0),
      U_CS2.io.result(0 + 13)
    ).asBool & CS2_round_up) |
      (Efp_b_is_greater & U_CS3.io.result.head(1).asBool & Mux(
        res_is_f32,
        U_CS3.io.result(0),
        U_CS3.io.result(0 + 13)
      ).asBool & CS3_round_up)
  )
  ...
  NX_reg := RegNext(
    Mux(
      (sel_CS2 & (U_CS2.io.result
        .head(1)
        .asBool & Mux(res_is_f32, U_CS2.io.result(0), U_CS2.io.result(0 + 13)).asBool & !CS2_round_up)) |
        (sel_CS3 & (U_CS3.io.result
          .head(1)
          .asBool & Mux(res_is_f32, U_CS3.io.result(0), U_CS3.io.result(0 + 13)).asBool & !CS3_round_up)) | sel_CS4,
      B_guard,
      0.U
    )
  )
  close_fraction_result_reg := Mux(RegNext(sel_CS4), CS4_reg.tail(1), CS_0123_lshift_result_reg.tail(1))
  val lshift_result_head_is_one_reg = ((RegNext(EA) - 1.U > RegNext(lzd_0123)) & RegNext(CS_0123_result.orR)) | RegNext(
    (mask_onehot & CS_0123_result).orR
  )
...
private[this] class ShiftLeftPriorityWithF64EXPResult(val srcW: Int, priorityShiftValueW: Int, expW: Int)
    extends Module {
  val io = IO(new Bundle() {
    val src = Input(UInt(srcW.W))
    val priority_shiftValue = Input(UInt(priorityShiftValueW.W))
  ...
  def shiftLeftPriorityWithF64EXPResult(srcValue: UInt, priorityShiftValue: UInt): UInt = {
    ...
    def do_shiftLeftPriority(srcValue: UInt, priorityShiftValue: UInt, i: Int): UInt = {
      if (i == 0)
        ...
        Mux(
          priorityShiftValue(i),
          if (i == width - 1) Cat(srcValue(i, 0), "b01110000000".U - (width - i - 1).U(11.W))
          else Cat(Cat(srcValue(i, 0), 0.U((width - 1 - i).W)), "b01110000000".U - (width - i - 1).U(11.W)),
          do_shiftLeftPriority(srcValue = srcValue, priorityShiftValue = priorityShiftValue, i = i - 1)
        )
    }
    do_shiftLeftPriority(srcValue = srcValue, priorityShiftValue = priorityShiftValue, i = width - 1)
...
private[this] class FarPathAdderF64WidenPipeline(val AW: Int, val AdderType: String, val stage0AdderWidth: Int = 0)
    extends Module {
  val io = IO(new Bundle() {
    val A = Input(UInt(AW.W))
    val B = Input(UInt(AW.W))
...
private[this] class FarPathFloatAdderF64WidenPipeline(
  ...
  val B_wire =
    Mux(absEaSubEb_is_greater, Fill(significandWidth + 1, EOP), far_rshift_widen_result.head(significandWidth + 1))
  val B_guard_normal_reg = RegNext(
    Mux(
      absEaSubEb_is_greater,
      false.B,
      Mux(
  ...
  val B_round_normal_reg = RegNext(
    Mux(
      absEaSubEb_is_greater,
      false.B,
      Mux(
  ...
  val B_sticky_normal_reg = Mux(
    ...
    Mux(
      RegNext(EOP),
      RegNext(~far_rshift_widen_result.tail(significandWidth + 3)).asUInt.orR,
      RegNext(far_rshift_widen_result.tail(significandWidth + 3)).orR
    )
  )
  val B_rsticky_normal_reg = B_round_normal_reg | B_sticky_normal_reg
  val B_guard_overflow_reg = RegNext(
    Mux(
      absEaSubEb_is_greater,
      false.B,
      Mux(
  ...
  val U_FS0 = Module(
    new FarPathAdderF64WidenPipeline(AW = significandWidth + 1, AdderType = "FS0", stage0AdderWidth = 0)
  )
  U_FS0.io.A := A_wire
  U_FS0.io.B := B_wire
  val FS0 = U_FS0.io.result
  val U_FS1 = Module(
    new FarPathAdderF64WidenPipeline(AW = significandWidth + 1, AdderType = "FS1", stage0AdderWidth = 0)
  )
  ...
  val grs_overflow = Mux(
    RegNext(EOP),
    Cat(RegNext(A_wire(0)), 0.U, 0.U) - Cat(RegNext(~B_wire(0)), B_guard_normal_reg, B_rsticky_normal_reg),
    Cat(FS0(0), B_guard_normal_reg, B_rsticky_normal_reg)
  )
  ...
  far_exponent_result := Mux(
    far_case_overflow | (FS1
      .head(1)
      .asBool & FS0(0) & far_case_normal_round_up) | (RegNext(!EA.orR) & FS0.tail(1).head(1).asBool),
    RegNext(EA_add1),
    RegNext(EA)
  )
  ...
  io.fp_c := Mux(OF, result_overflow, Cat(far_sign_result_reg, far_exponent_result, far_fraction_result))
  if (hasMinMaxCompare) io.isEfp_bGreater := isEfp_bGreater
  else io.isEfp_bGreater := 0.U
}
...
private[this] class ClosePathFloatAdderF64WidenPipeline(
  ...
  val B_guard = Mux(
    Efp_b_is_greater,
    Mux(Efp_a_is_not_zero, fp_a_mantissa(0), false.B),
  ...
  U_CS4.io.adder_op1 := RegNext(
    Mux(
      ...
      Cat(0.U, significand_fp_b(significandWidth - 1, 1))
    )
  )
  val CS4_reg = U_CS4.io.result
  ...
  val sel_CS1 = exp_is_equal & U_CS0.io.result.head(1).asBool
  val sel_CS2 =
    !exp_is_equal & !Efp_b_is_greater & ((!U_CS2.io.result.head(1).asBool | !U_CS2.io.result(0).asBool) | !CS2_round_up)
  val sel_CS3 =
    !exp_is_equal & Efp_b_is_greater & ((!U_CS3.io.result.head(1).asBool | !U_CS3.io.result(0).asBool) | !CS3_round_up)
  val sel_CS4 = !exp_is_equal & (
  ...
  val lshift_result_head_is_one =
    ((RegNext(EA - 1.U) > RegNext(lzd_0123)) & RegNext(CS_0123_result).orR) | RegNext(mask_onehot & CS_0123_result).orR
...
private[vector] class FloatAdderF64WidenPipeline(val is_print: Boolean = false, val hasMinMaxCompare: Boolean = false)
    extends Module {
  val exponentWidth = 11
  val significandWidth = 53
  val floatWidth = exponentWidth + significandWidth
...
//  )
  val fp_a_to64 = Mux(io.res_widening, io.widen_a, io.fp_a)
...
//  )
  val fp_b_to64 = Mux(io.res_widening & !io.opb_widening, io.widen_b, io.fp_b)

  val EOP = (fp_a_to64.head(1) ^ io.is_sub ^ fp_b_to64.head(1)).asBool
  val U_far_path = Module(
    new FarPathFloatAdderF64WidenPipeline(
      exponentWidth = exponentWidth,
      significandWidth = significandWidth,
      is_print = is_print,
      hasMinMaxCompare = hasMinMaxCompare
    )
  )
  ...
  U_far_path.io.round_mode := io.round_mode
  val U_close_path = Module(
    new ClosePathFloatAdderF64WidenPipeline(
      exponentWidth = exponentWidth,
      significandWidth = significandWidth,
      is_print = is_print,
      hasMinMaxCompare = hasMinMaxCompare
  ...
  val fp_a_is_NAN = io.fp_aIsFpCanonicalNAN | Efp_a_is_all_one & fp_a_mantissa_isnot_zero
  val fp_a_is_SNAN =
    io.fp_aIsFpCanonicalNAN | Efp_a_is_all_one & fp_a_mantissa_isnot_zero & !fp_a_to64(significandWidth - 2)
  val fp_b_is_NAN = io.fp_bIsFpCanonicalNAN | Efp_b_is_all_one & fp_b_mantissa_isnot_zero
  val fp_b_is_SNAN =
    io.fp_bIsFpCanonicalNAN | Efp_b_is_all_one & fp_b_mantissa_isnot_zero & !fp_b_to64(significandWidth - 2)
  val fp_a_is_infinite = Efp_a_is_all_one & (!fp_a_mantissa_isnot_zero)
  val fp_b_is_infinite = Efp_b_is_all_one & (!fp_b_mantissa_isnot_zero)
  val fp_a_is_zero = Efp_a_is_zero & !fp_a_mantissa_isnot_zero
  val fp_b_is_zero = Efp_b_is_zero & !fp_b_mantissa_isnot_zero
  ...
  val is_far_path_reg = RegNext(
    !EOP | absEaSubEb(absEaSubEb.getWidth - 1, 1).orR | (absEaSubEb === 1.U & (Efp_a_is_zero ^ Efp_b_is_zero))
  )
  val float_adder_fflags = Wire(UInt(5.W))
  val float_adder_result = Wire(UInt(floatWidth.W))
  when(RegNext((fp_a_is_SNAN | fp_b_is_SNAN) | (EOP & fp_a_is_infinite & fp_b_is_infinite))) {
    ...
    ) | ((fp_b_is_zero_reg | fp_a_is_zero_reg) & res_widening_reg)
  ) {
    float_adder_fflags := "b00000".U
  }.otherwise {
  ...
  when(RegNext(fp_a_is_NAN | fp_b_is_NAN | (EOP & fp_a_is_infinite & fp_b_is_infinite))) {
    ...
    float_adder_result := RegNext(
      Cat(
        Mux(fp_a_is_infinite, fp_a_to64.head(1), io.is_sub ^ fp_b_to64.head(1)),
        Fill(exponentWidth, 1.U),
        Fill(significandWidth - 1, 0.U)
      )
    )
  }.elsewhen(res_widening_reg & fp_a_is_zero_reg & fp_b_is_zero_reg) {
    float_adder_result := RegNext(
      Cat(Mux(io.round_mode === "b010".U & EOP | (fp_a_to64.head(1).asBool & !EOP), 1.U, 0.U), 0.U(63.W))
    )
...
private[this] class FarPathAdderF16Pipeline(val AW: Int, val AdderType: String, val stage0AdderWidth: Int = 0)
    extends Module {
  val io = IO(new Bundle() {
    val A = Input(UInt(AW.W))
    val B = Input(UInt(AW.W))
...
private[vector] class FloatAdderF16Pipeline(val is_print: Boolean = false, val hasMinMaxCompare: Boolean = false)
    extends Module {
  ...
  val absEaSubEb = U_far_path.io.absEaSubEb
  val is_far_path =
    !EOP | absEaSubEb(absEaSubEb.getWidth - 1, 1).orR | (absEaSubEb === 1.U & (Efp_a_is_not_zero ^ Efp_b_is_not_zero))
  val is_close_path = EOP & (!absEaSubEb(absEaSubEb.getWidth - 1, 1).orR)
  val fp_a_mantissa = io.fp_a.tail(1 + exponentWidth)
  val fp_b_mantissa = io.fp_b.tail(1 + exponentWidth)
  val fp_a_mantissa_isnot_zero = io.fp_a.tail(1 + exponentWidth).orR
  ...
  val fp_a_is_NAN = io.fp_aIsFpCanonicalNAN | Efp_a_is_all_one & fp_a_mantissa_isnot_zero
  val fp_a_is_SNAN =
    io.fp_aIsFpCanonicalNAN | Efp_a_is_all_one & fp_a_mantissa_isnot_zero & !io.fp_a(significandWidth - 2)
  val fp_b_is_NAN = io.fp_bIsFpCanonicalNAN | Efp_b_is_all_one & fp_b_mantissa_isnot_zero
  val fp_b_is_SNAN =
    io.fp_bIsFpCanonicalNAN | Efp_b_is_all_one & fp_b_mantissa_isnot_zero & !io.fp_b(significandWidth - 2)
  val fp_a_is_infinite = Efp_a_is_all_one & (!fp_a_mantissa_isnot_zero)
  val fp_b_is_infinite = Efp_b_is_all_one & (!fp_b_mantissa_isnot_zero)
  val float_adder_fflags = Wire(UInt(5.W))
  val float_adder_result = Wire(UInt(floatWidth.W))
  ...
  when(RegNext(fp_a_is_NAN | fp_b_is_NAN | (EOP & fp_a_is_infinite & fp_b_is_infinite))) {
    ...
    float_adder_result := Cat(
      RegNext(Mux(fp_a_is_infinite, io.fp_a.head(1), io.is_sub ^ io.fp_b.head(1))),
      Fill(exponentWidth, 1.U),
      Fill(significandWidth - 1, 0.U)
    )
  }.otherwise {
