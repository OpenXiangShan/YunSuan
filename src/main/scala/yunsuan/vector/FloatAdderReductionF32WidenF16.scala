...
class FloatAdderReductionF32WidenF16(val is_print: Boolean = false) extends Module {
  ...
  U_Stage_N.io.stageN_in := RegNext(U_Stage_N.io.stageN_out.asUInt)
    .asTypeOf(new StageNInputBundleF32Widen(exponentWidth = exponentWidth, significandWidth = significandWidth))
  ...
  def shiftLeftPriorityWithLZDResult(srcValue: UInt, priorityShiftValue: UInt): UInt = {
    ...
    def do_shiftLeftPriority(srcValue: UInt, priorityShiftValue: UInt, i: Int): UInt = {
      if (i == 0)
        ...
        Mux(
          priorityShiftValue(i),
          if (i == width - 1) Cat(srcValue(i, 0), (width - i - 1).U(lzdWidth.W))
          else Cat(Cat(srcValue(i, 0), 0.U((width - 1 - i).W)), (width - i - 1).U(lzdWidth.W)),
...
private[vector] class FloatAdderReductionF32WidenF16StageN(
  val floatType:        String = "f16",
  var exponentWidth:    Int = 0,
  var significandWidth: Int = 0,
  ...
  val U_far_B_rshift = Module(
    new FarShiftRightWithMuxInvFirst(far_smallerSignificand_widen.getWidth, farmaxShiftValue.getWidth)
  )
  ...
  val far_B =
    Mux(absEaSubEb_is_too_greater, Fill(significandWidth + 1, EOP), far_rshift_widen_result.head(significandWidth + 1))
  ...
  val grs_overflow = Mux(
    EOP,
    Cat(far_A_0, 0.U, 0.U) - Cat(~far_B_0, B_guard_normal, B_rsticky_normal),
    Cat(FS0_0, B_guard_normal, B_rsticky_normal)
  )
  ...
  far_fraction_result := Mux1H(
    ...
    Seq(
      Cat(
        ...
        Mux(res_is_fp16, FS0_0 ^ far_case_normal_round_up, FS0(13)),
        FS0(12, 1),
        FS0_0 ^ far_case_normal_round_up
      ),
      Cat(FS1(significandWidth - 2, 1 + 13), Mux(res_is_fp16, 0.U, FS1(13)), FS1(12, 1), 0.U),
      Cat(
        FS0(significandWidth - 1, 2 + 13),
        Mux(res_is_fp16, FS0_1 ^ far_case_overflow_round_up, FS0(14)),
        FS0(13, 2),
        FS0_1 ^ far_case_overflow_round_up
      ) |
  ...
  val priority_lshift_mask_1 =
    CS1 | Mux(close_absEaSubEb, Mux(isEfp_bGreater, mask_Efp_b_onehot, mask_Efp_a_onehot), mask_Efp_b_onehot)
  ...
  val sel_CS0 = (!close_absEaSubEb & !U_CS0.io.result
    .head(1)
    .asBool) | (close_absEaSubEb & U_CS1.io.result.head(1).asBool & round_up)
  val sel_CS1 = !sel_CS0
  val U_Lshift_0 = Module(
    new ShiftLeftPriorityWithLZDResult(srcW = CS0.getWidth, priorityShiftValueW = priority_lshift_mask_0.getWidth)
  )
  ...
  val U_Lshift_1 = Module(
    new ShiftLeftPriorityWithLZDResult(srcW = CS1.getWidth, priorityShiftValueW = priority_lshift_mask_1.getWidth)
  )
  ...
  U_fp_result_Mux1H.io.in := VecInit(
    Mux(
      res_is_fp16,
      Cat(0.U(17.W), Fill(5, 1.U), 1.U, Fill(11 - 2, 0.U)),
      Cat(0.U, Fill(exponentWidth, 1.U), 1.U, Fill(significandWidth - 2, 0.U))
    ),
    Mux(
      res_is_fp16,
      Cat(0.U(16.W), Mux(fp_a_is_infinite, close_fp_a_sign, close_fp_b_sign), Fill(5, 1.U), Fill(11 - 1, 0.U)),
      Cat(
        Mux(fp_a_is_infinite, close_fp_a_sign, close_fp_b_sign),
    ...
    Mux(
      res_is_fp16,
      Cat(0.U(16.W), Cat(close_result.head(1), close_result(27, 23), close_result(22, 13))),
      close_result
    )
  )
  ...
  val normal_fraction_result =
    Mux(is_far_path, far_fraction_result, close_fraction_result) & Cat(Fill(10, 1.U), Fill(13, !res_is_fp16))
  ...
  val U_fp_n_is_denormal_to_f32 = Module(
    new ShiftLeftPriorityWithF32EXPResult(srcW = 10, priorityShiftValueW = 10, expW = 8)
  )
  ...
  val next_fp_a_sign =
    Mux(is_init, fp_0_to32.head(1).asBool, Mux(res_is_fp16, io.fp_result(15), io.fp_result.head(1).asBool))
  ...
  val next_significand_fp_a = Cat(
    ...
    Mux(is_init, fp_0_to32.tail(exponentWidth + 1), normal_fraction_result)
  )
  ...
  val U_EA_sub_EN_sub_lzd = Module(
    new Adder(EA_sub_EN_add_1.getWidth, EA_sub_EN_add_1.getWidth, EA_sub_EN_add_1.getWidth, is_sub = false)
  )
  U_EA_sub_EN_sub_lzd.io.a := EA_sub_EN_add_1
  U_EA_sub_EN_sub_lzd.io.b := (~lzd_01.asTypeOf(EA_sub_EN_add_1)).asUInt
  val U_EN_sub_EA_add_lzd = Module(
    new Adder(EN_sub_EA.getWidth, EN_sub_EA.getWidth, EN_sub_EA.getWidth, is_sub = false)
  )
  ...
  val next_close_smaller_significand_lsb =
    Mux(res_is_fp16, next_smallerSignificand(0 + 13).asBool, next_smallerSignificand(0).asBool)
  ...
  val next_mask_Efp_a_onehot = Cat(
    ...
    (for (i <- 2 until significandWidth + 2) yield (next_Efp_a === i.U).asUInt).reduce(Cat(_, _))
  )
  val next_mask_Efp_b_onehot = Cat(
    next_Efp_b === 0.U | next_Efp_b === 1.U,
    (for (i <- 2 until significandWidth + 2) yield (next_Efp_b === i.U).asUInt).reduce(Cat(_, _))
  )
  ...
  val next_fp_b_mantissa_isnot_zero =
    Mux(io.res_widening | res_is_fp16, io.fp_n.tail(16 + 6).orR, next_fp_b_mantissa.orR)
  ...
  val next_is_far_path = !next_far_EOP | next_absEaSubEb(exponentWidth - 1, 1).orR | (next_absEaSubEb(
    0
  ) === 1.U & (next_Efp_a_is_not_zero ^ next_Efp_b_is_not_zero))
  ...
  if (is_print) {
    ...
    printf(
      p"io.stageN_out.far_smallerSignificand_widen_1bit = ${Binary(io.stageN_out.far_smallerSignificand_widen_1bit)}\n"
    )
