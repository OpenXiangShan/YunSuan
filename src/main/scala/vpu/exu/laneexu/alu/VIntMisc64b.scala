...
class VIntMisc64b extends Module {
  val io = IO(new Bundle {
    ...
    // val fs1 = Input(UInt(FLEN.W)) // for vfmv.s.f  Remove 16.2 2025.03.31
  ...
  perm_vmv := Mux1H(Seq(
    ...
    // (funct3 === "b101".U) -> Mux(sew.oneHot(3), io.fs1, VFPU.unbox(io.fs1, VFPU.S))  remove 16.2 2025.03.31
  ))
  ...
  io.rd.bits := Mux1H(Seq(
    ...
    // (funct3 === "b001".U) -> Mux(sew.oneHot(3), vs2(FLEN-1, 0), Cat(~0.U(32.W), vs2(31, 0))),  remove 16.2 2025.03.31
  ))
